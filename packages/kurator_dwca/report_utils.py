...
def term_corrector_report(inputfile, reportfile, vocabfile, key, separator='|', 
    format=None):
    ...
    ### Required parameters ###
    if reportfile is None or len(reportfile)==0:
        logging.debug('No reportfile name given in term_corrector_report().')
        return False
    ...
    if key is None or len(key.strip())==0:
        s = 'No key given in term_corrector_report().'
        logging.debug(s)
        return False
    ...
    if vocabfile is None or len(vocabfile) == 0:
        logging.debug('No vocabulary file given in term_corrector_report().')
        return False

    if os.path.isfile(vocabfile) == False:
        s = 'Vocabulary file %s not found in term_corrector_report().' % vocabfile
        logging.debug(s)
        return False
    ...
    if len(vocabdict) == 0:
        ...
        s += 'had zero recommendations in term_corrector_report().'
        logging.debug(s)
        return False
