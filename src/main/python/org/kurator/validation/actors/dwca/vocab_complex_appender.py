...
__version__ = "vocab_complex_appender.py 2015-10-20T16:05:27+01:00"
...
# Example: 
#
# kurator -f workflows/vocab_complex_appender.yaml -p p=fullpath -p v=../../vocabularies/basisOfRecord.csv -p n='a, b, c'
#
# or as a command-line script.
# Example:
#
# python vocab_complex_appender.py -i ../../vocabularies/day.csv -n '33'

# Global variable for the list of potentially new values for the term to append to the vocab file
newvaluelist = None

def vocab_complex_appender(inputs_as_json):
    """Given a set of distinct key values for a given term complex, append any not already 
...
def main():
    """
    Example: 
    python vocab_complex_appender.py -i ../../vocabularies/dwcgeography.csv -s , -n "|United States|California|||||, North America|United States|Washington|Chelan||||"
    """
    options = _getoptions()
    ...
    print 'newvaluelist: %s' % newvaluelist
    if fullpath is None:
        plist = "'|United States|California|||||', 'North America|United States|Washington|Chelan||||'"
        print "syntax: python vocab_complex_appender.py -i ../../vocabularies/dwcgeography.csv -s , -n %s" % plist
        return
    ...
    response=json.loads(vocab_complex_appender(json.dumps(inputs)))

#    print 'Response: %s' % response
    logging.info('To file %s, added new values: %s' % (fullpath, response['addedvalues']))

if __name__ == '__main__':
    """ Demo of vocab_complex_appender"""
    main()
